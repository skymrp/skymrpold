...
pub struct SliceRegisterContext<'a> {
    regs: &'a mut [u32],
}

impl<'a> SliceRegisterContext<'a> {
    pub fn new(regs: &'a mut [u32]) -> Self {
        Self { regs }
    }
}

impl RegisterContext for SliceRegisterContext<'_> {
    fn read_reg(&mut self, reg: AbiReg) -> u32 {
        self.regs[reg.index()]
    }
...
/// * The syscall dispatcher ([crate::syscall]) creates a stub guest function that the
///   guest code can call.
